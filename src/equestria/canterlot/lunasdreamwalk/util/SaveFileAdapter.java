...
public class SaveFileAdapter {
	...
	public SaveFileAdapter(byte[] fileContent) throws IOException {
		...
		// calculation
		this.setSecondKey(Util
				.hexStringToByteArray("302A75507ACBD72F89504E4712901CF0"));
	}
	...
	public boolean sanityCheck() {
		return getVersionNumber() == 1 && payloadDecompressedSize() > 0
				&& payloadCompressedSize() > 0
				&& payloadEncryptedSize() == payload().length;
	}
	...
	public boolean encryptSecondLayer() {
		...
		byte[] encrypted = Util.encrypt(
				Util.stringToASCIIByteArray(this.xmlContent.toString()),
				secondKey);
	...
	public boolean encryptFirstLayer() {
		...
		byte[] newCompressed = new byte[compressed.length + 8
