...
public class Util {
     ...
     */
    public static int parseInteger(byte[] data, int offset) throws ArrayIndexOutOfBoundsException {

        if(data.length < offset + 4)
            throw new ArrayIndexOutOfBoundsException("Not enough bytes for Reading an Integer at " + offset);
    ...
    public static byte[] encrypt(byte[] data, byte[] key) {
        ...
        for(; a > 0; a--) {
            ...
            for(int b = 0; b < d; b++) {
                l = s[(b + 1) % d];
                m = (j >>> 5 ^ l << 2) + (l >>> 3 ^ j << 4) ^ (h ^ l) + (g[b & 3 ^ i] ^ j);
                j = s[b] += m;
            }
        }
     ...
     *             if (offset > data.length - 4)
     */
    public static void writeInteger(byte[] data, int offset, int value) throws ArrayIndexOutOfBoundsException {
        if(data.length < offset + 4) {
            throw new ArrayIndexOutOfBoundsException("Not enough space for writing an Integer at " + offset);
        }
    ...
    public static byte[] hexStringToByteArray(String hexString) {
        ...
        for(int i = 0; i < target.length; i++) {
            target[i] = (byte) Integer.parseInt(hexString.substring(i * 2, i * 2 + 2), 16);
